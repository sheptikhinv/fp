...
public static class AutofacDependencyInjection
{
    ...
    public static ContainerBuilder AddWordsFilter(this ContainerBuilder builder, string? filterFilePath)
    {
        builder.Register<IBoringWordsFilter>(c =>
        {
            ...
            var reader = factory.GetReader(filterFilePath);
            if (!reader.IsSuccess)
                return new LengthBoringWordsFilter();
            var words = reader.Value.ReadWords(filterFilePath);
            if (!words.IsSuccess)
                return new LengthBoringWordsFilter();

            return new SpecifiedBoringWordsFilter(words.Value);
    ...
    public static ContainerBuilder AddCoordinateGenerators(this ContainerBuilder builder, double angleStep = 0.1f)
    {
        builder.Register(c => new SpiralCoordinateGenerator(angleStep)).As<ICoordinateGenerator>();
